...
AS
    s_no_reserved_places INT;
    BEGIN
        SELECT COUNT(*)
        INTO s_no_reserved_places
        FROM V_Reservations2 VR
        INNER JOIN TRIPS T ON T.NAME = VR.TRIP_NAME
        WHERE T.TRIP_ID = s_trip_id;

        UPDATE TRIPS
            SET TRIPS.NO_AVAILABLE_PLACES = TRIPS.NO_PLACES - s_no_reserved_places
        WHERE TRIP_ID = s_trip_id;
    END;

--sprawdzenie procedury calculate_available
declare
	S_TRIP_ID NUMBER := 26;
begin
	CALCULATE_AVAILABLE_PLACES(S_TRIP_ID => S_TRIP_ID);
end;
...
BEGIN
    ...
    stat   := :new.STATUS;
    INSERT INTO RESERVATION_LOG VALUES(DEFAULT, res_id, trunc(SYSDATE), stat);
END;
...
-- sprawdzenie działania triggera modify_reservation
declare
	MODIFY_RESERVATION_ID NUMBER := 7;
	MODIFY_STATUS CHAR(1) := 'C';
begin
	MODIFY_RESERVATION_STATUS3(
		MODIFY_RESERVATION_ID => MODIFY_RESERVATION_ID,
		MODIFY_STATUS => MODIFY_STATUS
	);
end;
...
CREATE OR REPLACE TRIGGER CHANGE_STATUS
    ...
    FOR EACH ROW
BEGIN
   INSERT INTO RESERVATION_LOG(RESERVATION_ID, CHANGE_DATE, STATUS) VALUES(:new.RESERVATION_ID, trunc(sysdate)
            , :new.STATUS);
END;


CREATE OR REPLACE TRIGGER UPDATE_AVAILABLE
    BEFORE UPDATE OF NO_PLACES
    ON TRIPS
    FOR EACH ROW
BEGIN
       :new.NO_AVAILABLE_PLACES := :old.NO_AVAILABLE_PLACES + :new.NO_PLACES - :old.NO_PLACES;
END;
...
declare
	S_TRIP_ID NUMBER := 21;
	NEW_NO_PLACES NUMBER := 30;
begin
	MODIFY_NUMBER_PLACES4(
		S_TRIP_ID => S_TRIP_ID,
		NEW_NO_PLACES => NEW_NO_PLACES
	);
end;
